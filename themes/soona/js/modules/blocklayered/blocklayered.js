...
function openCloseFilter()
{
	$('#layered_form span.layered_close a').live('click', function(e)
	{
		if ($(this).html() == '&lt;')
		{
			$('#'+$(this).attr('rel')).show(600); // Megnor For Toogle Speed 
			$(this).html('v');
			$(this).parent().removeClass('closed');
		}
		else
		{
			$('#'+$(this).attr('rel')).hide(600);// Megnor For Toogle Speed 
			$(this).html('&lt;');
	...
	ajaxQuery = $.ajax(
	{
		...
		success: function(result)
		{
			...
			if ($.cookie("grid-list") == 'list_view') {
				...
				$(".list_view li").css('width', ''); 
				$(".list_view li").css('height', ''); 
				$('.list_view .product-block').css("height", "");
				$('.list_view .product-block').css("width", "");
