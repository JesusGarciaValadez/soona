...
{if $special}
		<ul class="products clearfix">
			<li class="product_image">
				<a href="{$special.link}">
					<img src="{$link->getImageLink($special.link_rewrite, $special.id_image, 'small_default')|escape:'html'}" alt="{$product.legend|escape:html:'UTF-8'}" title="{$special.name|escape:html:'UTF-8'}" />
					</a>
			</li>
...
{else}
		<p class="no-products">{l s='No product specials are available at this time.' mod='blockspecials'}</p>
{/if}
	</div>
</div>
<!-- /MODULE Block specials -->