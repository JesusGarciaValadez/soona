...
<div id="best-sellers_block_right" class="block products_block clearfix">
	<p class="title_block"><a href="{$link->getPageLink('best-sales')|escape:'html'}" title="{l s='View a top sellers products' mod='blockbestsellers'}">{l s='Top sellers' mod='blockbestsellers'}</a></p>
	...
	{if $best_sellers|@count > 0}
		<ul class="product_images">
			...
			<li class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if} clearfix">
				...
				</a><span class="price">{$product.price}</span></p>
			</li>
		{/foreach}
		</ul>
