<?php
class TmHomeSlide extends ObjectModel
{
	public $title;
	public $url;
	public $image;
	...
	public function add($autodate = true, $null_values = false)
	{
		...
		$res &= Db::getInstance()->execute('
			INSERT INTO `'._DB_PREFIX_.'tmhomeslider` (`id_shop`, `id_homeslider_slides`)
	...
	public function delete()
	{
		...
		{
			if (preg_match('/main-banner/', $image) === 0)
	...
	public function reOrderPositions()
	{
		...
		$rows = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			LEFT JOIN `'._DB_PREFIX_.'tmhomeslider` hs ON (hss.`id_homeslider_slides` = hs.`id_homeslider_slides`)
			WHERE hs.`id_shop` = '.(int)$id_shop.' AND hss.`position` > '.(int)$this->position
		);

		foreach ($rows as $row)
		{
			$current_slide = new TmHomeSlide($row['id_slide']);
